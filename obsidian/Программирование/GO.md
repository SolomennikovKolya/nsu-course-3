...
В Go функция `make` используется для выделения и инициализации некоторых встроенных типов данных: срезов (slices), карт (maps) и каналов (channels). В отличие от функции `new`, которая просто выделяет память и возвращает указатель, `make` создаёт и возвращает инициализированное значение этих типов данных.
