...
#### Несколько куки
Браузер сохраняет новую куки отдельно, если следующая комбинация уникальна:
- `Name` (имя куки)
- `Domain` (домен, включая поддомены)
- `Path` (путь на сервере)
Иначе куки перезаписывается
...
**CSS** (Cascading Style Sheets) — это язык стилей, который управляет внешним видом HTML-элементов. Он отвечает за: шрифты, цвета, отступы, тени, анимации;  адаптивность под устройства; поведение при наведении, клике и т.д.

CSS состоит из правил вида:
`селектор { свойство: значение; }` // селектор - то к чему применяется правило

```css
/* Селекторы */
p               { margin: 0; }           /* по тегу */
#header         { background: blue; }    /* по id */
.container      { padding: 20px; }       /* по классу */
a:hover         { color: orange; }       /* псевдокласс */
ul > li         { list-style: none; }    /* вложенность */

/* Свойства */
width, height, margin, padding, color, background, font-size, display, flex, grid, position, transition, animation, etc.

```

#### Основные варианты интеграции CSS

| Вариант                | Пример использования                                  | Особенности                                                       |
| ---------------------- | ----------------------------------------------------- | ----------------------------------------------------------------- |
| Inline-стили           | `<div style={{ color: 'red' }}>`                      | Быстро, но не масштабируется                                      |
| Встроенный в HTML      | `<style> ... </style>` - внутри head в HTML           | Полезно, чтобы браузер сразу же видел стили при загрузке страницы |
| Внешний CSS            | `<link rel="stylesheet" href="style.css">` - в head   | Удобно для разделения HTML и CSS                                  |
| Глобальные CSS-файлы   | `index.css` подключается так: `import './index.css';` | Стили применимы ко всему приложению                               |
| CSS-модули             | `Component.module.css`                                | Изоляция стилей на уровне компонента                              |
| CSS-in-JS (библиотеки) | `styled-components`, Emotion                          | Стили как часть логики — динамика, темы                           |
| Utility-классы         | Tailwind CSS                                          | Нет `.css` вообще, стили в `className`                            |
| Препроцессоры          | SCSS, LESS                                            | Переменные, вложенность, миксины                                  |
| Постпроцессоры         | PostCSS                                               | Автопрефиксы, minify, кастомные правила                           |
