...
struct _Queue
{
	...
	pthread_cond_t cond_non_empty; // Условная переменная для ожидания условия, что очередь не пустая
	pthread_cond_t cond_non_full;  // Условная переменная для ожидания условия, что очередь не полная
...
queue_t *queue_init(int max_count)
{
	...
	if (pthread_cond_init(&q->cond_non_empty, NULL))
	{
		perror("queue_init: pthread_create() failed: Failed to initialize condition variable");
		abort();
	}
	if (pthread_cond_init(&q->cond_non_full, NULL))
	{
		perror("queue_init: pthread_create() failed: Failed to initialize condition variable");
		abort();
	}
